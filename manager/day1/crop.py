	...
	for cnt in contours:
		...
		if len(approx)==4 and cv2.contourArea(cnt) > 10000:
			...
			if(xloc>region[0] and xloc<region[1]):
				...
				reg = 3
			#print reg
			sign = np.zeros(64,dtype = np.float32)
			sign = grid_signature(binarize(temp_post))
			#print signature

			post_name =  'post' + str(noOfPostIts+label) + '.jpg'
			txt_name =  'post' + str(noOfPostIts+label) + '.json'
		
			j=json.dumps({'label':noOfPostIts+label,'signature':sign.tolist() , 'x':xloc, 'y':yloc, 'column':reg},indent=4)
			f=open(txt_name,'w')
			f.write(j)
			f.close()
