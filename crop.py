...
#Original image must not be skew by 45 degrees or more.
...
def rotateYellows(image):
	...
	cv2.imwrite('yellow_dots.jpg',yellow_threshed)
...
def square_contours(threshed_image,original,name,region):
	...
	for cnt in contours:
		...
		if len(approx)==4 and cv2.contourArea(cnt) > 10000:
			...
			post_name = name + str(noOfPostIts) + '.jpg'
			txt_name = name + str(noOfPostIts) + '.json'
		
			j=json.dumps({'signature':sign[0:25:].tolist() , 'x':sign[25], 'y':sign[26], 'column':sign[27]},indent=4)
			f=open(txt_name,'w')
			f.write(j)
			f.close()
...
def blue_blobs(hsv_img,original,region):
	...
	b_name = 'blue'
	b_threshed,blues = square_contours(blue_threshed , original , b_name,region)
	cv2.imwrite('blue_contours.jpg',b_threshed)
	print blues 
	return blues

def pink_blobs(hsv_img,original,region):
	...
	p_name = 'purpink'
	p_threshed,purpinks = square_contours(pur_pink_threshed , original, p_name,region)
	cv2.imwrite('purpink_contours.jpg',p_threshed)
	print purpinks 
	return purpinks

def green_blobs(hsv_img,original,region):
	...
	g_name = 'green'
	g_threshed,greens = square_contours(green_threshed , original, g_name,region)
...
def blob(img):
	...
	cv2.imwrite('rotatedOriginal.jpg',img)
	region = column(img)
	original = img.copy()
	hsv = cv2.cvtColor(img,cv2.COLOR_BGR2HSV)	
	blue_posts = blue_blobs(hsv,img,region)
	purpink_posts = pink_blobs(hsv,img,region)
	green_posts = green_blobs(hsv,img,region)

	posts=json.dumps({'blues':blue_posts, 'purpinks':purpink_posts, 'greens':green_posts},indent = 4)
	f=open('noOfPosts.json','w')
	f.write(posts)
	f.close()


#Read image from which post-its will be cropped.
im_2_crop = cv2.imread('IMG-20130122-00399A.jpg')
